...
class Levelling(commands.Cog):
    ...
    @commands.command(help="Check your current level and rank throughout all server the bot is in.", aliases=['level', 'lvl'])
    async def rank(self, ctx, member: discord.Member = None):
        if member is None:
            member = ctx.author
        if member.bot:
            return await ctx.send(embed=await failure_embed(title="You can't check the rank of a bot."))
        user = await self.bot.db.get_user(user_id=member.id, guild_id=ctx.guild.id)
        if user is not None:
    ...
    async def leaderboard(self, ctx, *, lb_type=None):
        query = "SELECT id, messages_sent, garrix_coins, total_xp FROM users WHERE guild_id = $1"
        ...
        if lb_type in ["coins", "coin", "currency", "garrix coins"]:
            ...
            lb = [[ctx.guild.get_member(record['id']), ] for record in records]

        else:
            return await ctx.send(embed = discord.Embed(title=f"{'That category of leaderboard does not exist.' if lb_type is not None else 'Please specify a category for leaderboard'}", description=help_categories, color=discord.Colour.teal()))
