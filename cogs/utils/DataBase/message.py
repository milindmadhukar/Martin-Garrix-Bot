...
class Message(object):
    ...
    async def on_message(cls, bot, message: Discord_Message) -> None:
        ...
        guild_multiplier = await bot.db.fetchrow("SELECT xp_multiplier FROM guild_configs WHERE guild_id = $1", message.guild.id)
        ...
        if difference > datetime.timedelta(minutes=1) or user.last_xp_added is None:
            user.total_xp += (xp * guild_multiplier['xp_multiplier'])
