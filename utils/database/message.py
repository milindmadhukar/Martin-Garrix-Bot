...
class Message:
    ...
    async def on_message(
        cls, bot: commands.Bot, message: discord.Message, xp_multiplier: int
    ) -> None:
        ...
        if difference > datetime.timedelta(minutes=1) or user.last_xp_added is None:
            ...
            if before_message_level != after_message_level:
                ...
                if (
                    after_message_level >= bot.true_garrixer_level
                    and bot.true_garrixer_role not in message.author.roles
                ):
                    ...
                    await message.author.add_roles(
                        bot.true_garrixer_role,
                        reason=f"Reached level {bot.true_garrixer_level}",
                    )
